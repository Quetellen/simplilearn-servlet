...
/**
 * Servlet Filter implementation class _01_TokenValidateFilter
 */
public class _01_TokenValidateFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if ("POST".equals(method) && token.trim().isEmpty()) {
			/*
			 *If the token is empty or not present we display an error message to the user 
			 *and the request does not reach the server. 
			 */
			out.print("<html>");
	        out.print("<body>");
